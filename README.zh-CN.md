...
**官网:** [https://urlcomments.leesangwoo.com/](https://urlcomments.leesangwoo.com/) · [隐私政策](https://urlcomments.leesangwoo.com/privacy/) · [服务条款](https://urlcomments.leesangwoo.com/terms/)
...
### 1. 安装步骤 (加载已解压的扩展程序)
1. 从 [GitHub Releases](https://github.com/s4ngwoo/URLComments/releases) 下载最新版本的 **`URLComments-vX.X.X.zip`**。
2. 将下载的 ZIP 文件解压至本地文件夹。
3. 打开 Google Chrome 浏览器，在地址栏输入 `chrome://extensions`。
4. 开启右上角的 **开发者模式 (Developer mode)**。
5. 点击左上角的 **加载已解压的扩展程序 (Load unpacked)**，选择刚刚解压的文件夹。
6. 在浏览器右上角工具栏中固定 (Pin) URLComments 图标。

### 2. 使用方法
1. **浏览网页**: 打开任意想发表或查看评论的网页（新闻文章、技术博客、官方文档、电商网站等）。
2. **打开扩展**: 点击工具栏中的 URLComments 图标打开侧边栏或弹出窗口。
3. **Google 登录**: 点击 Google 登录按钮进行快速安全登录（仅需首次授权）。
4. **浏览评论**: 查看其他用户在当前规范化 URL 下留下的公开观点。
5. **发表与互动**:
   - 在底部输入框输入内容（最多 1,000 字）并点击 **发布**。
   - 点击评论下方的 `↳ 回复` 参与 1 层树状讨论。
   - 点击作者昵称旁边的 👍 / 👎 进行点赞或踩。
6. **我的评论与设置**: 使用底部导航栏查看您的历史评论，或自定义暗色模式、字体大小与界面语言。
...
热烈欢迎任何形式的贡献、Bug 报告和功能建议！

- **贡献指南**: 详细的本地开发环境搭建、代码规范与 PR 提交流程，请参阅 [docs/CONTRIBUTING.md](docs/CONTRIBUTING.md)。
- **架构概览**: 了解系统设计与隐私保障原则，请参阅 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)。
- **快速开始**:
  ```bash
  npm install              # 安装开发依赖
  npm test                 # 运行自动化测试套件 (Jest)
  npm test test/locales    # 校验 6 种语言本地化翻译键的一致性
  ```
- **隐私第一原则**: 所有提交的代码必须严格遵守隐私优先原则（严禁后台标签页监控、仅在用户显式打开弹出层时发起请求、严格执行 URL 规范化）。
